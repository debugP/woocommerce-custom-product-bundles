...
var dialogBox; // Declare dialogBox variable
...
function updateBundleTotal() {
    ...
    jQuery('.item-price:checked').each(function() {
        ...
        if (price) {
            ...
            var $item = jQuery(this).closest('li.product, tr'); // Look for either li.product or tr
            
            // Selectors to find product details within the parent element (li.product or tr)
            var title = $item.find('.product-name, td:nth-child(2) a').first().text().trim(); // Look for .product-name or link in the second td
...
function selectBundleItem(checkbox) {
    ...
    var $itemElement = $checkbox.closest('li.product, tr'); // Look for either li.product or tr
    
    // Apply selected-item class to the correct parent element (li or tr)
...
jQuery(document).ready(function($) {
    ...
    dialogBox = $( "#dialog-box" ).dialog({
        ...
        minWidth: 350,
        maxWidth: 500,
        position: { my: 'center', at: 'center', of: window } // Add centering option
    });
                    ...
                    // Handle errors (e.g., display messages)
                     if (response.error && response.error.messages) {
                         ...
                         $.each(response.error.messages, function(index, message) {
                             // Use the same alertValidationError function for consistency
