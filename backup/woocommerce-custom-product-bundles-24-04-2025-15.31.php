...
function thps_woo_perfume_therapy($atts) {
	...
	foreach ($subcategories_products as $title => $subcat_products):
		if($subcat_products->have_posts()){
			...
			$rare_fragrance_ids = thps_display_products_list($id, $subcat_products, $rare_fragrance_ids, 'product_variation', $title, $product_class, $img_width, $img_height, false, false, false, false, $show_img);
		}
	endforeach;
	...
	if($rare_fragrances->have_posts()){
		...
		thps_display_products_list($id, $rare_fragrances, '', 'product_variation', 'RARE FRAGRANCES', $product_class, $img_width, $img_height, false, true, true, false, $show_img);
	}
	wp_reset_query();
...
function thps_display_products_list(
    ...
    return $product_ids + ob_get_clean();
}
