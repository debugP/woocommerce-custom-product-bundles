...
function thps_woo_perfume_therapy($atts) {
    ...
    // Estrazione degli attributi del shortcode
    ...
    $show_img           = ($atts['show_img'] === true || (int)$atts['show_img'] === 1 || $atts['show_img'] === 'true') ? true : false;

    // Caricamento prodotto bundle
    $bundle_product = new WC_Product($atts['id']);
    $IDbyNAME       = get_term_by('name', $atts['category'], 'product_cat');
    $category_id    = $IDbyNAME->term_id;

    // Parametri di query per le sottocategorie
    $subcatsargs = array(
        'hierarchical'     => 1,
        'show_option_none' => '',
        'hide_empty'       => 0,
    ...
    $form_html = '<form id="thps_product_bundle_' . $atts['id'] . '" class="thps_product_bundle" method="post" enctype="multipart/form-data">';
    $form_html .= thps_modal_box();

    // Introduzione al questionario
    $form_html .= '<div class="questionnaire">';
    $form_html .= __('1. Indicate the essences that you would like to enter into the composition of your bespoke perfume', 'woocommerce-custom-product-bundles');
    $form_html .= '<p align="center">';
    $form_html .= sprintf(__('Maximum %d fragrances', 'woocommerce-custom-product-bundles'), $atts['max_fragrances']);
    $form_html .= '</p>';
    $form_html .= '</div>';

    $form_html .= '<div class="shop_columns_' . $atts['per_row'] . ' thps_shop_columns">';
    $form_html .= '<ul id="customPerfume" class="products thps-products">';

    // Caricamento dei prodotti per ogni sottocategoria
    $subcategories_products = array();
    foreach ($subcats as $subcat) {
        if (!in_array($subcat->slug, $exclude_categories)) {
            $parent_products_ids = thps_get_product_ids_by_category($subcat->slug);
    ...
    // Ordinamento dei prodotti per sottocategoria
    $subcategories_products = sortProductListByCount($subcategories_products);
    foreach ($subcategories_products as $title => $subcat_products) {
        if ($subcat_products->have_posts()) {
            $col_index = $index == 0 ? 0 : $index % $atts['per_row'];
            $product_class = ($col_index == 0) ? 'first' : (($col_index == ($atts['per_row'] - 1)) ? 'last' : '');
            $index++;

            // Display prodotti della sottocategoria
            $rare_fragrance_ids = thps_display_products_list($atts['id'], $subcat_products, $rare_fragrance_ids, 'product_variation', $title, $product_class, $atts['img_width'], $atts['img_height'], false, false, false, false, $show_img);
        }
    }
    ...
    if ($rare_fragrances->have_posts()) {
        ...
        // Display fragranze rare
        thps_display_products_list($atts['id'], $rare_fragrances, '', 'product_variation', 'RARE FRAGRANCES', $product_class, $atts['img_width'], $atts['img_height'], false, true, true, false, $show_img);
    }
    wp_reset_query();
    ...
    $form_html .= '</ul>';
    $form_html .= '</div>';
    $form_html .= build_questionnaire_html($qstn2, $qstn3, $qstn4, $qstn5, $qstn6, $email);

    $form_html .= '<input type="hidden" name="has_questionnaire" value="true" />';
    $form_html .= thps_actions_row($atts['bundle_name'], $bundle_product, 'perfume-therapy', $atts['min_fragrances'], $atts['max_fragrances']);
    $form_html .= '</form>';
...
function build_questionnaire_html($qstn2, $qstn3, $qstn4, $qstn5, $qstn6, $email) {
    ...
    $qstn_html .= '<br/>';
    $qstn_html .= '<textarea name="qstn2">' . htmlspecialchars($qstn2) . '</textarea>';
    $qstn_html .= '</div>';
    ...
    $qstn_html .= '<br/>';
    $qstn_html .= '<textarea name="qstn3">' . htmlspecialchars($qstn3) . '</textarea>';
    $qstn_html .= '</div>';
                        ...
                        <tr>
                            <td class="<?= $show_price_in_name ? 'product-name' : '' ?>">
                                ...
                                <span class="product-label">
                                    <a href="<?= esc_url($link) ?>" target="_blank"><?= esc_html($prod_name) ?></a>
                                </span>
                            </td>
